...
fn pebble_header_fixup(b: *std.Build, pebble_include_path: []const u8) []const u8 {
    var pebble_header = std.Io.Dir.cwd().readFileAlloc(
        b.graph.io,
        b.pathJoin(&.{ pebble_include_path, "pebble.h" }),
        b.allocator,
        .limited(1 << 20),
    ) catch |err| switch (err) {
        error.FileNotFound => std.debug.panic("'pebble.h' not found in '{s}'. Please set 'pebble_sdk_path' build option to the correct path.", .{pebble_include_path}),
        else => std.debug.panic("Error reading 'pebble.h' in '{s}' : {t}", .{ pebble_include_path, err }),
    };
...
pub fn addPebbleApplication(b: *std.Build, options: PebbleApplicationOptions) void {
    ...
    // Current pebble-tool uses XDG data directories; keep legacy installs working.
    const pebble_sdk_path = b.option([]const u8, "pebble_sdk_path", "Pebble SDK root (auto-detected)") orelse path: {
        const home = b.graph.environ_map.get("HOME") orelse
            @panic("Set -Dpebble_sdk_path to your Pebble SDK directory");
        const data_home = b.graph.environ_map.get("XDG_DATA_HOME") orelse b.pathJoin(&.{ home, ".local/share" });
        const candidates = [_][]const u8{
            b.pathJoin(&.{ data_home, "pebble-sdk/SDKs/current" }),
            b.pathJoin(&.{ home, ".pebble-sdk/SDKs/current" }),
            b.pathJoin(&.{ home, "Library/Application Support/Pebble SDK/SDKs/current" }),
        };
        for (candidates) |candidate| {
            std.Io.Dir.accessAbsolute(b.graph.io, b.pathJoin(&.{ candidate, "sdk-core" }), .{}) catch continue;
            break :path candidate;
        }
        @panic("Pebble SDK not found. Install it with `pebble sdk install 4.33.1` or set -Dpebble_sdk_path");
    };
    ...
    const run_upload_step = b.addSystemCommand(&.{ "uv", "tool", "run", "--from", "pebble-tool", "pebble", "install" });
